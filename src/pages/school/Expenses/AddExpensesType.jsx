...
import AddStudentCategory from "@/component/Institute/expenses/AddExpensesTyoe";
import UpdateCategory from "@/component/Institute/expenses/UpdateExpensesType";
...
function AddExpensesType() {
  ...
  const { expensestype, loading } = useSelector(
    (state) => state.GetExpensesType
  );
  ...
  const handledelete = () => {
    serverInstance("expenses/addexpensestype", "delete", {
      id: deleteid,
    }).then((res) => {
      if (res?.status === true) {
  ...
  const filter = () => {
    dispatch(GetExpensesType(Vehicletype));
  };

...
function AddTimeTable({updatedata, setOpen }) {
  ...
  useEffect(() => {
    if (updatedata) {
      let last = updatedata?.subject?.starttime.split(" ").pop();
      var lastIndex = updatedata?.subject?.starttime?.lastIndexOf(" ");
      let first = updatedata?.subject?.starttime?.substring(0, lastIndex);

      setsamorpm(last);
      setsh(first?.substring(0, 2));
      setsm(first?.substring(3, 5));

      let elast = updatedata?.subject?.endtime.split(" ").pop();
      var elastIndex = updatedata?.subject?.endtime?.lastIndexOf(" ");
      let efirst = updatedata?.subject?.endtime?.substring(0, elastIndex);

      seteamorpm(elast);
      seteh(efirst?.substring(0, 2));
      setem(efirst?.substring(3, 5));

      setdayname(updatedata?.subject?.dayname);
      setempID(updatedata?.subject?.empID);
      setclassId(updatedata?.subject?.classId);
      setsubject(updatedata?.subject?.subject);
    }
  }, []);

...
function UpdateOtherFee({ setOpen, updatedata }) {
  ...
  const submit = () => {
    setloading(true);
    serverInstance("student/otherfee", "put", {
      othersfeeobj:updatedata,
  ...
  useEffect(() => {
    ...
    if (updatedata) {
      ...
      setcoursename(updatedata?.Course);
      setcomment(updatedata?.OtherFeeName);
      setamount(updatedata?.FeeAmount);
      setduesDate(
        new Date(updatedata?.DuesDate).toISOString().substring(0, 10)
      );
    }
  }, []);

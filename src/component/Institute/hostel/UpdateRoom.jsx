...
function UpdateRoom({ setOpen, updatedata }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    setloading(true);
    serverInstance("hostel/addroom", "put", {
      id: updatedata?.id,
  ...
  useEffect(() => {
    if (updatedata) {
      ...
      setFacilityname(updatedata?.Facility);
      setfromroom(updatedata?.FromRoom);
      settoroom(updatedata?.ToRoom);
      setcomment(updatedata?.comment);
      setamountpermonth(updatedata?.PermonthFee);
    }
  }, []);
